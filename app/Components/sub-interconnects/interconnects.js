  ...
  return (
    <Fragment>
      <section className="max-w-6xl mx-auto">
        <h1 className="text-lg py-10 font-semibold">Interconnects (Components)</h1>
        <ul role="list" className="gap-x-4 sm:columns-1 md-sm:columns-2 md:columns-3 components">
          {data.length > 0 &&
            data.map (postdata => {
              return postdata.interconnects.components.map (item => {
                return (
 ...
 scrollamount="16" behavior="alternate" direction="left"> 
        ...
        {data.length > 0 &&
            data.map (postdata => {
              ...
              return postdata.interconnects.logos.map (item => {
                return (
